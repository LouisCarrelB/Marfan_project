...
class Atom:
	def __init__(self, x, y, z, numRes=-1, resType="X", tempfactor=0, sse="X"):
		self.x = x
		self.y = y
		self.z = z
...
def readPDB(nomFi, chaineVoulue="-", modelVoulu="1"):
	try:
		f = open(nomFi, "r")
	except IOError:
	...
	for ligne in f:
		if re_expdta.search(ligne):
			...
			if (numModel==None):
			    numModel=0
			if (a!=None and (numModel==0 or str(numModel)==modelVoulu) and c==chaineVoulue) :
				lesAtomes.append(a)
...
def writePDB(nomFichier, listeAtomes):
	fOUT = open(nomFichier,"w")
	N = len(listeAtomes)
	for i in range(N):
		fOUT.write("ATOM  %5d  CA  %s  %4d    %8.3f%8.3f%8.3f%6.2f%6.2f            \n"%(i+1,listeAtomes[i].resType,listeAtomes[i].numRes,listeAtomes[i].x,listeAtomes[i].y,listeAtomes[i].z,1.00,listeAtomes[i].tempfactor))
	fOUT.write("TER\n")
	fOUT.write("END\n")
	fOUT.close()
